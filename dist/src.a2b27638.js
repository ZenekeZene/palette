...
function convertCMYKtoRGB(cmyk) {
  ...
  return color; //return convert.cmyk.rgb(`${cmyk[0]}%`, `${cmyk[1]}%`, `${cmyk[2]}%`, `${cmyk[3]}%`);
}
...
function calculateSimilarCoefficient(cmyk) {
  ...
  for (var i = 0; i < swatchesHistorical.length; i++) {
    var summation = 0;

    for (var j = 0; j < cmyk.length; j++) {
      var absSimilar = Math.abs(cmyk[j] - swatchesHistorical[i][j]);
...
function getColorRelationed(cmyk) {
  ...
  for (var i = 0; i < cmyk.length; i++) {
    var percentage = Math.random(0, 1).toFixed(2);
    var result = parseInt(cmyk[i] * percentage, 10);
...
function addColors(color1, color2) {
  if (color1.length === color2.length) {
    ...
    for (var i = 0; i < color1.length; i++) {
      var add = parseInt(color1[i], 10) + parseInt(color2[i], 10);
...
function subtractColors(color1, color2) {
  if (color1.length === color2.length) {
    ...
    for (var i = 0; i < color1.length; i++) {
      var sub = parseInt(color1[i], 10) - parseInt(color2[i], 10);
...
function doMix(dropzone, index) {
  ...
  if (checkSuccess(index)) {
    ...
    var _searchCorrectSwatchA = searchCorrectSwatchAndDropzone(),
...
function searchCorrectSwatchAndDropzone() {
  var dropzoneWasCorrect, swatchWasCorrect;

  for (var i = 0; i < swatches.length; i++) {
    for (var j = 0; j < dropzones.length; j++) {
      var cmyk = color.addColors(activeColorObject.cmyk, dropzones[j].cmyk);
  ...
  return {
    dropzoneWasCorrect: dropzoneWasCorrect,
    swatchWasCorrect: swatchWasCorrect
  };
}

function getRandomEnabledItem() {
  var swatchActives = swatches.filter(function (swatch) {
...
function initSwatches(swatchesNodes) {
  ...
  for (var i = 0; i < swatchesNodes.length; i++) {
    var swatch = new color.ColorObject(color.getColorCMYKRandom(), swatchesNodes[i]);
    swatch.index = i;
...
function initDropzones(dropzoneNodes) {
  ...
  for (var i = 0; i < dropzoneNodes.length; i++) {
    dropzones.push(new color.ColorObject(color.getColorRelationed(swatches[i].cmyk), dropzoneNodes[i]));
  }
